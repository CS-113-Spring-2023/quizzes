...
\part[5] from $V_1$ to $V_2$,
  \begin{solution}
    We use Hall's theorem to prove that a complete matching \textit{exists} in this case. That is, we argue that for every subset of $V_1$, the condition in Hall's theorem holds.
    \begin{proof}
      \begin{itemize}
      \item We know that $|V_1| =2, |V_2| =3$, and $\forall v\in V_1 (N(v) = V_2)$.
      \item Then, $\forall A\subseteq V_1 (N(A)=V_2)$.
      \item $A$ is largest when $A=V_1$. Then $|A|=2$ and $N(A)=3$.
      \end{itemize}
    \end{proof}
  \end{solution}
\part[5] from $V_2$ to $V_1$.
  \begin{solution}
    We use Hall's theorem to prove that a complete matching \textit{does not exist} in this case. That is, we show a subset, $A$, of $V_2$ for which the condition in Hall's theorem does not hold.
    \begin{proof}
      \begin{itemize}
      \item We know that $|V_2| =3, |V_1| =2$, and $\forall v\in V_2 (N(v) = V_1)$.
      \item For the purpose of applying Hall's theorem, consider $A=V_2$.
      \item $N(A)=V_1$. So, $|A|=3$ and $|N(A)|=2$.
      \end{itemize}
    \end{proof}
